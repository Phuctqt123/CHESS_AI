...
def get_move_for_bot(bot_name, fen):
    ...
    if "Genetic" in bot_name:
        return get_best_move_genetic(fen, depth=3)
    elif "Alpha-Beta Pruning" in bot_name or bot_name == "Alpha-Beta":
        return get_best_move_alphabeta(fen, depth=3)
    elif "MCTS" in bot_name:
        return get_best_move_mcts(fen, iterations=50)
    return None
...
def play_one_game(white_bot, black_bot, game_num):
    ...
    print(f"  Ván {game_num}: ", end="", flush=True)
    ...
    while not board.is_game_over():
        ...
        if uci:
            ...
            # Mỗi 10 nửa-nước in 1 dấu chấm để biết chương trình vẫn đang chạy
            if moves_played % 10 == 0:
                print(".", end="", flush=True)
        else:
            print(" [Bot không trả về nước đi hợp lệ]", end="", flush=True)
            break
...
def run_matchup(bot1, bot2):
    ...
    for i in range(1, NUM_GAMES_PER_MATCHUP + 1):
        ...
        print(f"    Kết quả: {white} (Trắng) vs {black} (Đen) | {moves} nửa-nước | {res}")
